...
const Products = () => {
  ...
  {currentItems.map((item) => (
    <Link
      ...
      key={item.id}
      className="card block shadow-md hover:shadow-lg transition border border-gray-300 
            ...
            <div className="flex justify-center mt-4">
                {Array.from({ length: Math.ceil(filteredProducts.length / itemsPerPage) }, (_, index) => (
                    <button
                        ...
                        className={`px-4 py-2 mx-1 border ${currentPage === index + 1 ? "bg-green-500 text-white" : "bg-white text-black"}`}
                    >
                        {index + 1}
                    </button>
                ))}
